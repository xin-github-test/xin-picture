...
import com.xin.xinpicturebackend.utils.ColorSimilarUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public PictureVO uploadPicture(Object inputSource, PictureUploadRequest pictureUploadRequest, User loginUser) {
        ...
        if (spaceId != 0) {
            ...
            //校验是否有权限，仅空间管理员能上传，改为 sa-token 统一校验
...
//            }
    ...
    public List<PictureVO> searchPictureByColor(Long spaceId, String picColor, User loginUser) {
        ...
        if (!space.getUserId().equals(loginUser.getId())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR, "无空间权限访问！");
        }
    ...
    private void fillPictureWithNameRule(List<Picture> pictureIdList, String nameRule) {
        if (StrUtil.isBlank(nameRule) || CollUtil.isEmpty(pictureIdList)) {
            return;
        }
        long count = 1;
        try {
